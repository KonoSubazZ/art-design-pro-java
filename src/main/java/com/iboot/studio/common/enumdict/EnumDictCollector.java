...
public class EnumDictCollector {
    ...
    public EnumDictCollector(ApplicationContext applicationContext) {
    ...
    private void scanEnums() {
        ...
        // 检查重复的value
        valueToClasses.forEach((key, classes) -> {
            if (classes.size() > 1) {
                String duplicateClasses = classes.stream()
                    .map(Class::getName)
        ...
        valueToClasses.forEach((key, classes) -> {
            ...
            info.setOptions(Arrays.stream(enumClass.getEnumConstants())
                .map(enumConstant -> {
    ...
    private void findEnumClassesInDirectory(java.io.File directory, String packageName, Set<Class<?>> result) {
